	...
	for _, h := range options.Hosts {
		...
		for _, l := range lines {
			...
			case "zk_server_state":
				zkLeader := fmt.Sprintf("zk_server_leader{%s,state=%q}", hostLabel, value)
				if value == "leader" {
					metrics[zkLeader] = "1"
				} else {
					metrics[zkLeader] = "0"
				}
				zkState := fmt.Sprintf("zk_server_state{%s,state=%q}", hostLabel, value)
				metrics[zkState] = "1"
			case "zk_version":
		...
		if conn, err := dial(tcpaddr.String(), timeout, options.ClientCert); err == nil {
			res = sendZookeeperCmd(conn, h, "conf")
			lines := strings.Split(res, "\n")
			for _, l := range lines {
				if l == "" {
					continue
				...
				metrics[fmt.Sprintf("zk_conf_%s{%s}", key, hostLabel)] = value
			}

		}

		if conn, err := dial(tcpaddr.String(), timeout, options.ClientCert); err == nil {
			res = sendZookeeperCmd(conn, h, "srvr")
			lines := strings.Split(res, "\n")
			for _, l := range lines {
				if l == "" {
					continue
				...
				metrics[fmt.Sprintf("zk_srvr_%s{%s}", key, hostLabel)] = value
			}
